...
3. **Agent guides** are *built artifacts*: edit only `AGENT_GUIDE_TO_*.template.md`, then run `pnpm render:guides` to produce the builds and sync `guides.json`. Templates with language-specific content build to per-language `.ts.md` / `.swift.md`; concept-only templates (no lang blocks, no example placeholders) build to a single agnostic `.md` and list no variants in guides.json. **Never edit rendered builds directly.** Templates have two substitution mechanisms:
   ...
   - `{{#lang ts}} … {{/lang}}` / `{{#lang swift}} … {{/lang}}` — language-scoped prose (implementation notes, gotchas) rendered ONLY into that language's build. An iOS agent must never fetch JavaScript gotchas: language-specific prose goes in a lang block or doesn't exist. Shared concept prose stays neutral and unscoped. Platform gaps are never documented in either build — scope a one-language capability's section to that language's block and file the parity bug (see STYLE.md for the editorial rule, `.claude/ISSUE_FILING.md` for where/how to file).
