...
- **Framework glue is generic-first.** A section that integrates a framework (Vue/Pinia, SwiftUI/`PrimitiveApp`) must lead with the compiled, parity-enforced **client** operation (`{{ example: }}` to a normal corpus pair, or a reference to where it's already shown), then present the binding as the wrapper around it. The binding itself can't compile in the corpus harness (it depends on app-template types), so promote it to a corpus file marked `// nocompile`. When the same concept exists in both frameworks (e.g. a data-loader), keep it a parity pair. When it genuinely exists in only one language (a Pinia store has no Swift twin; a `PrimitiveAppState` subclass has no TS twin), also mark the file `// no-parity` and reference it from inside that language's `{{#lang}}` block — `examples.mjs` then skips the cross-language requirement and the renderer keeps it out of the other build. Never invent a fake twin just to satisfy parity.
