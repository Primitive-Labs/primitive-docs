...
This is the doc set's **one scheduled deep pass** — the nightly truing deliberately stays scoped to its changed window and skips the set audit, so the drift that accumulates between releases gets caught and fixed here:
...
- **Cross-page findings are applied here, not parked.** The publish PR is the review surface, so apply the audit's findings as edits on the publish branch — duplication consolidated to the owning page, inconsistencies resolved against source, boundary violations moved — overriding `docs-set-audit`'s standalone "surface as proposals" rule, and call each applied restructure out prominently in the report. Only a genuinely large reorganization (a page split/merge whose review would materially delay the release) may be deferred: **file it as its own standalone GitHub issue** in `Primitive-Labs/primitive-docs` (one issue per restructure per `.claude/ISSUE_FILING.md`; a pure doc reorganization files here, not in a source repo) and say so in the report. Everything fixed here flows back to `next` via Step 8's back-merge.

Fold the audit result into the report: clean bill, or the fixes and restructures applied here plus the issue numbers filed for anything deferred.
...
- **Gates arbitrate.** Drive them green against the production surface (Step 4's repin first). A slice relying on a merged-but-unpublished package API is reverted on the publish branch (it stays on `next`) and recorded as held. Apply Step 6's audit findings on the publish branch per that step's rules (flagging applied restructures in the report); file only genuinely large deferred reorganizations — and any parity gaps — as issues per `.claude/ISSUE_FILING.md`, and record the numbers.
- **The report is the PR body.** Write it to `.docs-publish-release-report.md` at the repo root. FIRST line: `PUBLISH_STATUS: ready` (branch built, committed, gates green — mergeable as-is) or `PUBLISH_STATUS: blocked — <reason>` (leave the best-effort branch committed for a human to take over; the workflow opens it as a draft PR). Then: the Step 5 release-coverage table, the Step 6 set-audit result (fixes and restructures applied, prominently flagged, plus issue numbers for anything deferred), the Step 7 docs-delta summary (developer audience — no branches/channels/stamps/truing mechanics), and any held or gap items.
