...
**The automated `docs-next-sync` PRs are in scope.** These are the truing PRs the `docs-next-sync` workflow opens automatically — titled `docs-next-sync: <library> changes (automated, <date>)`, one per nightly pass, each moving a submodule pointer in `library_repos/`, re-stamping `docs-sources.json`, and updating the affected pages/guides. Earlier versions of this skill held them; that was wrong. Review them in concert with the rest of the queue and merge them into `next` like any other PR (they get extra handling in Step 2 because they move submodules — see "Handling `docs-next-sync` PRs"). **Each nightly PR is a *delta*, not a superset**: it covers only the library window since the previous sync PR, so an open chain of them must land **oldest first**, and no PR in the chain is redundant merely because a newer one exists. They merge into `next` only — never to `main`.
...
2. **Superseded check — is the change already in `next`?** A sync PR's prose may already have landed (a hand edit or a one-off PR got there first). Grep `next` for the specific fact the PR changes. If it's already present (often in fuller wording), the PR is **superseded** — close it with a comment pointing at where the content already lives, and let the next sync PR in the chain carry the submodule bump forward. Don't merge a no-op-with-conflicts. **A newer sync PR does not supersede an older one** — nightly PRs are deltas, and the newer one deliberately excludes the older's window; superseded means the *content* verifiably already lives on `next`, never "a newer sync PR exists."
...
**Order the `docs-next-sync` PRs last**, after the ordinary editorial PRs they overlap have landed — that way you reconcile their facts onto the final `next` wording, not onto a base you're about to overwrite. Then land the sync chain **oldest-target-first** so the submodule only ever moves forward, expecting the stamp/pin conflict between consecutive ones — it's structural, not a defect. The per-PR loop: merge the oldest; rebase the next sync PR onto the updated `next`, resolving `docs-sources.json`/`guides.json`/the submodule gitlink to **that PR's own (newer) values** and its doc edits on top of the just-merged content; re-run its Step 2 gates; merge; repeat. (Use the "Landing a reconciled sync PR" force-push mechanics below to put each rebased state through its PR.) With nightly delta PRs this loop is the *normal* path whenever more than one is open, not an exception.
