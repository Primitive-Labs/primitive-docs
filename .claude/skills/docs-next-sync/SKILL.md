...
- **No git/PR mechanics.** Do not run `git commit`, `git push`, `git checkout -b`, `git merge`, or `gh pr create`/`gh pr merge`. Leave all changes in the working tree — the workflow commits them to the rolling PR branch and opens/updates the PR into `next`. Step 0.5 never runs here.
...
- **The report is the PR body.** Write it to `.docs-next-sync-report.md` at the repo root. FIRST line: `SYNC_STATUS: ready` or `SYNC_STATUS: blocked — <reason>`. Then the full Step 4 report: the per-commit disposition table, drafted larger changes flagged for review, issues filed, the `Fixes #NN` lines, and the set-audit findings with the fixes applied. If the pass made no doc changes: `SYNC_STATUS: ready`, say so, and reset the submodules so the tree is clean.
