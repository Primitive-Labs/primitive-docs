...
6. Report: a table of every scanned commit → disposition (`internal — no change` / `updated <pages>` / `promoted <fence> → corpus` / `proposed: <new section/page>` awaiting user input), plus the open issues touched (`closed #NN` / `still deferred — gated on <ref>`), plus the whole-set audit's findings and the fixes applied, plus anything the sources contradicted in the docs that you couldn't resolve.
