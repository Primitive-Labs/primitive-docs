...
- **No "CRDT" (or other esoteric implementation jargon).** Documents merge concurrent edits automatically — describe *that*, not the data structure behind it: "simultaneous edits merge cleanly, with no conflicts." Where you need a noun, the plain word is "document data" / "local document state," not "CRDT" or a merge-algorithm name. Same principle as infrastructure internals: name the developer-facing behavior, not the implementation. Unlike the rest of this list, this one **also applies to the agent guides** — use "document store" / "merge semantics" there too.
...
- **TOML examples write native TOML, never JSON stuffed into a string.** Wherever a config surface accepts structured TOML — operation `definition`/`params` (`[operations.definition]` tables, `[[operations.params]]` entries), subscription `params`, workflow step object params (`[steps.params.<field>]`) — every example uses the nested-table (or inline-table) form. Never teach `definition = '{"filter":...}'` by example, in either product. The JSON-string encoding gets at most a prose mention (readers will meet it in files written before `primitive sync migrate-toml`), with no example block. The one exception is a surface whose *platform contract* is a JSON-encoded string (e.g. prompt test-case fields like `inputVariables`) — document those as the platform defines them, verified against the CLI's serializers.
