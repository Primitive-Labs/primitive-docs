...
{{/lang}}
{{#lang swift}}
**Wiring the acceptance flow:** Resolve the incoming invite URL (universal link / custom scheme) with `client.links` — `resolve(userActivity:)` / `resolve(url:)` returns a `.invitation(token:)` target carrying the `inviteToken` (see the [Authentication guide](AGENT_GUIDE_TO_PRIMITIVE_AUTHENTICATION.md#deep-links-and-universal-links)). When the user is signed in, call `client.invitations.accept(inviteToken)`. When they're signed out, hold the token and pass it to the sign-in method that carries it: `auth.magicLinkVerify(token:inviteToken:)`, `auth.otpVerify(email:code:inviteToken:)`, or `signInWithGoogle(..., inviteToken:)` — deferred grants then resolve to the signing-in user. With `signInWithApple(presentationAnchor:)`, hold the token through sign-in and call the authenticated `client.invitations.accept(inviteToken:)` immediately after. Cover the error state (`INVITE_TOKEN_INVALID` — one code for all bad tokens). See the [Authentication guide](AGENT_GUIDE_TO_PRIMITIVE_AUTHENTICATION.md) for token persistence across the sign-in round-trip.
{{/lang}}
