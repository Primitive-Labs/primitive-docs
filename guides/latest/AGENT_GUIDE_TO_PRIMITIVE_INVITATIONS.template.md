...
{{/lang}}
{{#lang swift}}
**Wiring the acceptance flow:** Handle the invite URL (universal link / custom scheme) to extract the `inviteToken`. When the user is signed in, call `client.invitations.accept(inviteToken)`. When they're signed out, hold the token and pass it to the sign-in method that carries it: `auth.magicLinkVerify(token:inviteToken:)`, `auth.otpVerify(email:code:inviteToken:)`, or `signInWithGoogle(..., inviteToken:)` — deferred grants then resolve to the signing-in user. Native `signInWithApple(presentationAnchor:)` does **not** carry an invite token; for that path, sign in first and then call the authenticated `client.invitations.accept(inviteToken:)`. Cover the error state (`INVITE_TOKEN_INVALID` — one code for all bad tokens). See the [Authentication guide](AGENT_GUIDE_TO_PRIMITIVE_AUTHENTICATION.md) for token persistence across the sign-in round-trip.
{{/lang}}
