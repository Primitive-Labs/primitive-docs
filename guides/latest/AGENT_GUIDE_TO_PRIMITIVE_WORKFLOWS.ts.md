...
`runAs` declares the principal a run executes as. Top-level `[workflow]` field, applied by both `primitive sync push` and `primitive workflows create --from-file`, validated at save time (`runAs` must be `caller` | `system`; absent → `caller`).
