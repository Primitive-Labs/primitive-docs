...
- **Script bodies are live-resolved.** The runner reads `Script.activeConfigId` on every execution — push a changed `.rhai` with `primitive sync push` and all referencing workflows pick up the new body on their next run; no workflow re-push required. To pin a specific revision for determinism, add `configId = "<id>"` to the step (mirrors `prompt.execute` pinning; the pinned config must still be `status = "active"` to execute).
