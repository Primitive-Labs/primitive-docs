...
type SidebarItem = {
  ...
  items?: SidebarItem[]
  /**
   * VitePress group options:
   * - collapsible: show a toggle to collapse/expand the group
   * - collapsed: initial collapsed state (when collapsible is true)
   */
  collapsible?: boolean
  collapsed?: boolean
}
...
function buildReferenceSidebar(tree: SidebarItem[]): SidebarItem[] {
  ...
  function pickTypedocApiSections(
    ...
    for (const title of sectionTitles) {
      ...
      if (section) out.push({ text: title, items: section.items ?? [], collapsible: true, collapsed: true })
    }
    return out
  }
  ...
  function pickPrimitiveApp(projectNode: SidebarItem | undefined): SidebarItem[] {
    ...
    const comps = componentsRoot
      ? [{ text: 'Components', items: componentsRoot.items ?? [], collapsible: true, collapsed: true }]
      : []
    ...
    const layouts = layoutsFolder
      ? [{ text: 'Layouts', items: layoutsFolder.items ?? [], collapsible: true, collapsed: true }]
      : []
    ...
    const pages = pagesFolder
      ? [{ text: 'Pages', items: pagesFolder.items ?? [], collapsible: true, collapsed: true }]
      : []
    ...
    if (stores) out.push({ text: 'Stores', items: stores.items ?? [], collapsible: true, collapsed: true })
    out.push(...layouts)
    out.push(...pages)
    out.push(...comps)
    if (services) out.push({ text: 'Services', items: services.items ?? [], collapsible: true, collapsed: true })
    if (router) out.push({ text: 'Router', items: router.items ?? [], collapsible: true, collapsed: true })
    if (utils) out.push({ text: 'Utils', items: utils.items ?? [], collapsible: true, collapsed: true })
    if (composables)
      out.push({ text: 'Composables', items: composables.items ?? [], collapsible: true, collapsed: true })
    if (config) out.push({ text: 'Config', items: config.items ?? [], collapsible: true, collapsed: true })
  ...
  if (primitive)
    items.push({ text: 'primitive-app', items: pickPrimitiveApp(primitive), collapsible: true, collapsed: true })
  if (wss)
    items.push({
