...
Define your models in a single `schema.toml`, run `swift-bao-codegen` at build time, and use the generated type's static `Model.*` API (`TaskRecord.query()`, `TaskRecord.create(_:in:)`, …). This replaces both the hand-rolled `BaoModelRecord` / `BaoModel<T>` path **and** the per-doc `TypedModel<T>` binding — the schema is the single source of truth for field names, Swift types, and wire format, so drift between the layers can't happen.
...
The codegen'd type **is** the model — there's no per-doc handle to bind. You just open your document(s); reads and writes go through the static `Model.*` API. Open your per-user singleton doc after connect:

```swift
@MainActor
final class MyAppState: PrimitiveAppState {
    // Pre-register so a doc opened before the first query is mirrored.
    // (Optional — the statics lazily register on first use.)
    override var crossDocumentModels: [any PrimitiveModel.Type] { [TaskRecord.self] }
...
Reads go through the static `Model.*` API and span **every open document by default** (the Swift mirror of the JS client's `Model.query()`). For SwiftUI views, bind a `BaoDataLoader<[T]>` to the `.onModel(subscribe:)` trigger — it owns the subscription lifecycle, debounces bursts, and re-runs your load closure on local **and** remote changes:
...
struct TaskListView: View {
    ...
    var body: some View {
        List(loader.data ?? []) { Text($0.title) }
            .task {
                loader.bind(
                    client: appState.client,
                    subscribeTo: [.onModel(subscribe: TaskRecord.subscribe)]
                ) { _ in
                    TaskRecord.query().sorted { $0.createdAt > $1.createdAt }
                }
            }
    }
}
...
Reads are **synchronous** against the local CRDT — no async/await. The `load` closure is `@MainActor`-isolated, so they compile under strict concurrency directly:

```swift
TaskRecord.find("task_123")                              // -> TaskRecord?
TaskRecord.findAll()                                     // -> [TaskRecord]
TaskRecord.query(["completed": false])                   // MongoDB-style predicate
TaskRecord.query(["completed": false],
                 options: QueryOptions(sort: ["createdAt": -1], limit: 50))
TaskRecord.count(["completed": true])
TaskRecord.query(nil, options: QueryOptions(documents: [docId]))  // scope to one doc
```

These span every open document by default; pass `options.documents` to scope to one or some. The client keeps one shared store per model and auto-connects each document on open, disconnecting on close (set up for you in a `PrimitiveApp` host via `JsBaoClient.configureDefault` during `initialize()`). `TypedModel` / `MultiDocModel` / `DynamicModel` are internal plumbing — app code uses the `Model.*` statics.

::: tip Why BaoDataLoader and not @Published?
You could keep a `@Published var items: [TaskRecord]` on the state class and call a `refresh()` method after every mutation. It works — but it drifts silently the moment you add a new mutation site and forget to call `refresh()`. `BaoDataLoader` + `.onModel(subscribe:)` removes the bookkeeping entirely: every add/update/delete on the model fires the trigger.
:::

### Write

Writes go through the static `Model.*` API too. They name their target document by id (writes throw if it isn't open) and are **local-first** — applied to the CRDT immediately, synced in the background:

```swift
try TaskRecord.create(TaskRecord(title: "Buy milk"), in: docId)   // throws on schema validation OR doc-not-open
try TaskRecord.update("task_123", ["completed": .boolean(true)], in: docId)
try TaskRecord.delete("task_123", in: docId)
```

`create` validates the new record (required fields, type coercibility, unique constraints) before inserting. `update` takes a partial `[String: PrimitiveValue]` (unknown keys dropped silently). All three throw if `docId` isn't open.
