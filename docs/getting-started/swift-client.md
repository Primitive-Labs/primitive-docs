...
Once a document is open, build a `TypedModel<T>` from it using `appState.makeTypedModel(...)`. One `TypedModel` per record type per document. Going through `makeTypedModel` (instead of constructing `TypedModel<T>(doc:)` directly) also registers the model with the in-app debug inspector.
...
let tasks: TypedModel<TaskRecord> = appState.makeTypedModel(doc: doc, documentId: documentId)
```

Most apps cache `TypedModel` instances on `PrimitiveAppState` (or a subclass) keyed by document ID, so views can reach them through `@EnvironmentObject`:
...
Reads are **synchronous** and hit the local CRDT — no async/await needed.
...
// Create + atomic alias (lets you find this doc later without storing the ID)
